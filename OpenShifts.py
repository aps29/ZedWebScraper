...
driver.get('https://zed.rutgers.edu/scheduling/open_shifts/')
...
for i in range(dropDownCount): 
    ...
    owner = soup.find_all("div", style = "width: 100%; text-align: center;")


    campus = soup.find("option", selected = True)

    # results.write("\n")
    # results.write(campus.text.strip() + ": ")
    # results.write("\n")
                ...
                for a in range(colspanCount):
                    ...
                    if(childrenNum == 0):
                        ...
                        for b in range(childrenNum):
                            if("Posted by:" in str(owner[ownerTimeCount].text.strip())):
                                # results.write("Day: " + day[dayCount].text.strip() + " - " + owner[ownerTimeCount].text.strip() + " - Start Time: " + startTime[ownerTimeCount].text.strip())
                                displayText += ("Day: " + day[dayCount].text.strip() + " - " + owner[ownerTimeCount].text.strip() + " - Start Time: " + startTime[ownerTimeCount].text.strip())
                                displayText += "\n"
                                ownerTimeCount += 1
                            else: 
                                ownerTimeCount += 1
